...
function NavSocialIcon({ icon }: { icon: string }) {
  if (icon === "max") {
    return (
      <span
        aria-hidden="true"
        className="inline-block h-[22px] w-[22px] bg-current [mask-image:url(/icons/max.svg)] [mask-position:center] [mask-repeat:no-repeat] [mask-size:contain]"
      />
    );
  }
  ...
  if (icon === "vk") {
    return (
      <svg aria-hidden="true" className="h-6 w-6" fill="none" viewBox="1.5 4 21 13.5">
